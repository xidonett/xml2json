...
class ConverterController extends Controller
{
    ...
    public function requestValidation(Request $request, $language)
    {
        ...
        if ( $validator->fails() )
            return back()->with("error", "File must have a .$language extension.");
    }
    ...
    {
        Storage::disk('open')->put("converted/{$request->filename}.{$request->extension}", $request->converted);
        $file = public_path()."/converted/{$request->filename}.{$request->extension}";
